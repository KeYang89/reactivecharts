...
var formItem = function formItem(_ref) {
	...
	return (/*#__PURE__*/(0, _emotion.css)(vh, ';&:focus{+ label{&::before{box-shadow:0 0 0 2px ', (0, _utils.shade)(theme.colors.primaryColor, 0.4), ';}}}&:hover{+ label{&::before{border-color:', theme.colors.primaryColor, ';}}}&:active{+ label{&::before{transition-duration:0;}}}+ label{position:relative;user-select:none;display:flex;width:100%;height:100%;align-items:center;cursor:pointer;&::before{background-color:#fff;border:2px solid ', theme.colors.borderColor || (0, _utils.shade)(theme.colors.textColor, 0.2), ';box-sizing:content-box;content:"";color:', theme.colors.primaryColor, ';margin-right:calc(', item.width, ' * 0.5);top:50%;left:0;width:', item.width, ';height:', item.height, ';display:inline-block;vertical-align:middle;}&::after{box-sizing:content-box;content:"";background-color:', theme.colors.primaryColor, ';position:absolute;top:50%;left:calc(2px + ', item.scale, '/2);width:calc(', item.width, ' - ', item.scale, ');height:calc(', item.height, ' - ', item.scale, ');margin-top:calc(', item.height, '/-2 - ', item.scale, '/-2);transform:scale(0);transform-origin:50%;transition:transform 200ms ease-out;}}')
	);
};
